...
import { file, write, spawn, Glob } from "bun";
...
async function editYaml(path: string, data: { [key: string]: any }) {
  ...
  const details: any = raw.trim() ? YAML.parse(raw) : {};
...
async function initWorkspace(path: string) {
  const tasks = [
    ...
    await write(
      ...
      await file(resolve(__dirname, "../", ".prettierrc")).text(),
    ),

    write(
      resolve(path, ".prettierignore"),
      await file(resolve(__dirname, "../", ".prettierignore")).text(),
    ),
